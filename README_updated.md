...
The repository is organized as a sequential workflow (`step1` through `step16`). The early steps construct the genetically supported drug-target cohort and ancestry representation variables; the middle steps fit the launch/approval analyses and robustness models; and the final steps evaluate direct-disease feasibility, cross-ancestry effect portability, and colocalization.
...
├── step16/                 # Final robustness and sensitivity analyses
└── figures/
    ├── publication_v5_code/
    └── publication_v5/
```
...
# 7. Step 16 — Final robustness and sensitivity analyses

Step 16 evaluates whether the main ancestry–launch and portability conclusions are robust to temporality, alternative measures of evidence volume, participant weighting, estimator behavior, and statistical precision.

Run from the repository root:

```bash
python3 step16/16a_landmark_analyses.py
python3 step16/16b_cumulative_participant_sample_size.py
python3 step16/16c_ancestry_breadth_vs_evidence_volume.py
python3 step16/16d_participant_weighted_ancestry.py
python3 step16/16e_cross_ancestry_sign_concordance.py
python3 step16/16f_perfect_portability_null_calibration.py
python3 step16/16g_portability_precision_restrictions.py
```

### 16A — Prospective landmark analyses

Reconstructs ancestry breadth and study volume using only evidence available by 2012, 2014, 2016, or 2018 and tests subsequent launch status. The ancestry association remains null after study-volume adjustment at each landmark.

### 16B — Cumulative participant sample size

Uses cumulative GWAS participant count as an alternative measure of evidence volume.

Expected checkpoint:

```text
Pairs with usable cumulative N:                  735
Unadjusted ancestry OR:                         ~1.13
Ancestry OR adjusted for cumulative N:          ~1.00
Adjusted cumulative-participant-N OR:           ~1.33
```

### 16C — Ancestry breadth versus evidence volume

Quantifies how ancestry breadth increases with the amount of supporting genetic evidence.

```text
Ancestry breadth vs study count:              Spearman rho ~0.62
Ancestry breadth vs cumulative participants:  Spearman rho ~0.51
```

### 16D — Participant-weighted ancestry

Constructs participant-weighted ancestry measures, including effective ancestry number and non-European participant share. Strict analyses use fully assignable ancestry records; equal-split analyses provide a sensitivity analysis for multi-ancestry records.

### 16E — Cross-ancestry sign concordance

Summarizes directional consistency among the exact LD-pruned variants used for portability.

```text
Independent ancestry comparisons:      104
LD-pruned variant comparisons:         714
Pooled sign concordance:              ~74.5%
Lead-variant direction concordance:   ~82.7%
```

### 16F — Perfect-portability null calibration

Tests whether the portability estimation procedure could generate the observed attenuation when the true cross-ancestry slope is one. The simulation preserves the exact 714 LD-pruned variant-comparisons and ancestry-specific uncertainty structure and uses 1,000 replicates with seed 42.

```text
Observed median slopes:
naive:   ~0.624
FIQT:    ~0.634
Deming:  ~0.627

Perfect-portability null medians:
naive:   ~0.99
FIQT:    ~1.01
Deming:  ~1.00
```

The observed median is below the simulated perfect-portability distribution for all three estimators (empirical lower-tail P approximately 0.001).

### 16G — Portability precision restrictions

Repeats portability summaries after restricting to comparisons with more retained variants or relatively favorable non-European precision.

```text
FIQT median slope
All primary comparisons:                ~0.63
At least 5 variants:                    ~0.60
At least 10 variants:                   ~0.57
Best sample-size-balance quartile:      ~0.67
Best standard-error-balance quartile:   ~0.67
```

The non-European datasets remain substantially less powered than the European references. No primary comparison has a non-European effective sample size at least 25% of its European counterpart, and none has a median non-European/EUR standard-error ratio of two or less. These restrictions are therefore sensitivity analyses rather than comparisons of equally powered ancestry-specific GWAS.
...
# 9. Key reproducibility checkpoints
...
15G → 15G2 → 15G3
  ↓
Step 16A → 16B → 16C → 16D → 16E → 16F → 16G
  ↓
Publication figures
```
